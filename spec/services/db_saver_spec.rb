...
RSpec.describe DBSaver do
  ...
  context 'does not persist' do
    ...
    it 'if tuple with diamond_code already exists' do
      valid_comic_hash[:diamond_id] = '11111'
      expect { db_saver.persist_to_db valid_comic_hash }.not_to change { Comic.count }
    end

    it 'if the publisher is DYNAMIC FORCES' do
      valid_comic_hash[:publisher] = 'DYNAMIC FORCES'
      expect { db_saver.persist_to_db valid_comic_hash }.not_to change { Comic.count }
    end
  end
end
